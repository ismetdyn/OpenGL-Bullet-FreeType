...
void initGame() {
    ...
    flatShaderProgram = shader.createProgram(
    ...
    textProgram = shader.createProgram(
            "Assets/Shaders/text.vs",
            "Assets/Shaders/text.fs");

    TextureLoader tLoader;

    sphereTexture = tLoader.getTextureID(
        "Assets/Textures/globe.jpg");
    groundTexture = tLoader.getTextureID(
    ...
    if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) {
        ...
        if (gameover) {
            ...
            if (grounded == true) {
                ...
                sphere->rigidBody->applyImpulse(btVector3(0.0f, 120.0f, 0.0f),
                    btVector3(0.0f, 0.0f, 0.0f));
                printf("Yukari tusunna basildi\n");
            }
        }
    }
}
...
void addRigidBodies()
{
    ...
    btCollisionShape* groundShape = new
        btBoxShape(btVector3(4.0f, 0.5f, 4.0f));
    ...
    btDefaultMotionState* motionState = new
        btDefaultMotionState(btTransform(
            btQuaternion(0, 0, 0, 1), btVector3(18.0, 1.0f, 0)));
    btRigidBody::btRigidBodyConstructionInfo rbCI(
        0.0f, motionState, shape, btVector3(0.0f, 0.0f, 0.0f));
    ...
    enemy = new MeshRenderer(MeshType::kCube, "enemy", camera,
        rb, light, 0.1f, 0.5f);
